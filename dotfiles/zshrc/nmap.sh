...
if grep -q "# \[NMAP.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[NMAP.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo
    echo "# [NMAP.SH] STARTED ---------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
...
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  echo "# [NMAP.SH] nmap plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${nmap_plugin[*]}" >> "$TARGET_ZSHRC"
fi

if ! grep -q "# \[NMAP.SH\] APPLIED" "$TARGET_ZSHRC"; then
  {
    echo
    echo "# [NMAP.SH] APPLIED ---------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
