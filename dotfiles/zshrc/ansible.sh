...
if grep -q "# \[ANSIBLE.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[ANSIBLE.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo ""
    echo "# [ANSIBLE.SH] STARTED ------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi

ansible_plugin=(ansible)
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  # 필수 플러그인이 배열에 없으면 추가
  for plugin in "${ansible_plugin[@]}"; do
  ...
  echo "# [ANSIBLE.SH] opentofu plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${ansible_plugin[*]}" >> "$TARGET_ZSHRC"
fi

# 성공 마커가 없으면 삽입
if ! grep -q "# \[ANSIBLE.SH\] APPLIED" "$TARGET_ZSHRC"; then
  echo "# [ANSIBLE.SH] APPLIED ------------------------------------------------------- #" >> "$TARGET_ZSHRC"
fi
