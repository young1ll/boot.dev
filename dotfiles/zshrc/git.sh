...
if grep -q "# \[GIT.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[GIT.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo
    echo "# [GIT.SH] STARTED ----------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
...
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  echo "# [GIT.SH] git plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${git_plugin[*]}" >> "$TARGET_ZSHRC"
fi
