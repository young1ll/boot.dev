...
if grep -q "# \[KUBECTL.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[KUBECTL.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo
    echo "# [KUBECTL.SH] STARTED ------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi

kubectl_plugin=(kubectl)
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  # 필수 플러그인이 배열에 없으면 추가
  for plugin in "${kubectl_plugin[@]}"; do
  ...
  echo "# [KUBECTL.SH] kubectl plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${kubectl_plugin[*]}" >> "$TARGET_ZSHRC"
fi
