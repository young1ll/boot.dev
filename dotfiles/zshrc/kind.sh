...
if grep -q "# [KIND.SH] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[KIND.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo ""
    echo "# [KIND.SH] STARTED ---------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
...
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  echo "# [KIND.SH] kind plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${kind_plugin[*]}" >> "$TARGET_ZSHRC"
fi

# 성공 마커가 없으면 삽입
if ! grep -q "# \[KIND.SH\] APPLIED" "$TARGET_ZSHRC"; then
  {
    echo "# [KIND.SH] APPLIED ---------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
