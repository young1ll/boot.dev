...
if grep -q "# \[ASURECLI.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[ASURECLI.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo ""
    echo "# [ASURECLI.SH] STARTED ------------------------------------------------------ #"
  } >> "$TARGET_ZSHRC"
fi

# aws 플러그인 추가
azure_plugin=(azure-cli)
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  echo "# [ASURECLI.SH] azure-cli plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${azure_plugin[*]}" >> "$TARGET_ZSHRC"
fi
