...
if grep -q "# \[BUN.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi
...
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  echo "# [BUN.SH] bun plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${bun_plugin[*]}" >> "$TARGET_ZSHRC"
fi

# 성공 마커가 없으면 삽입
if ! grep -q "# \[BUN.SH\] APPLIED" "$TARGET_ZSHRC"; then
  echo "# [BUN.SH] APPLIED ----------------------------------------------------------- #" >> "$TARGET_ZSHRC"
fi
