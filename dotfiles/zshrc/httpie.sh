...
if grep -q "# \[HTTPIE.SH\] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[HTTPIE.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo
    echo "# [HTTPIE.SH] STARTED -------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi

# aws 플러그인 추가
httpie_plugin=(httpie)
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  # 필수 플러그인이 배열에 없으면 추가
  for plugin in "${httpie_plugin[@]}"; do
