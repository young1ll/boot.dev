...
if grep -q "# [PODMAN.SH] APPLIED" "$TARGET_ZSHRC"; then
  exit 0
fi

# 헤더 블록이 없는 경우에만 삽입
if ! grep -q "# \[PODMAN.SH\] STARTED" "$TARGET_ZSHRC"; then
  {
    echo ""
    echo "# [PODMAN.SH] STARTED -------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
...
if grep -q '^plugins=' "$TARGET_ZSHRC"; then
  ...
  # 필수 플러그인이 배열에 없으면 추가
  for plugin in "${podman_plugin[@]}"; do
  ...
  echo "# [PODMAN.SH] podman plugin has added above" >> "$TARGET_ZSHRC"
else
  # plugins=() 라인이 아예 없는 경우
  printf "plugins=(%s)\n" "${podman_plugin[*]}" >> "$TARGET_ZSHRC"
fi

# 성공 마커가 없으면 삽입
if ! grep -q "# \[PODMAN.SH\] APPLIED" "$TARGET_ZSHRC"; then
  {
    echo "# [PODMAN.SH] APPLIED -------------------------------------------------------- #"
  } >> "$TARGET_ZSHRC"
fi
